...
\hypersetup{colorlinks=true, linkcolor=titleBlue, urlcolor=emsiGreen, citecolor=emsiGreen,
  pdftitle={AutoFlow --- Rapport de stage}, pdfauthor={ }}
...
\begin{minipage}{0.42\textwidth}\centering
  \includegraphics[width=6.5cm]{logoEmsi.png}
\end{minipage}\hfill
\begin{minipage}{0.42\textwidth}\centering
  \includegraphics[width=6.5cm]{honoris_logo.png}
\end{minipage}
...
{\Large\textsc{\color{emsiGreen}\textbf{Ecole Marocaine des Sciences de l'Ingenieur}}}\\[0.15cm]
{\normalsize\textit{Honoris United Universities --- EMSI Rabat}}\\[0.2cm]
{\large\textbf{Filiere : Intelligence Artificielle \& Science des Donnees}}\\[0.05cm]
{\large 4\textsuperscript{e} Annee --- 4IASD}
...
\begin{tcolorbox}[width=0.78\textwidth, colback=white, colframe=emsiGreen, arc=4pt, boxrule=2pt,
  halign=center, shadow={2pt}{-2pt}{0pt}{black!20}]
  \textbf{\large Rapport de Stage}\\[0.25cm]
  \textbf{\large $\triangleright$ Projet \textit{AutoFlow} v1.0}
\end{tcolorbox}

\vspace{1cm}

{\Huge\bfseries\color{titleBlue}
  AutoFlow : Systeme d'Automatisation\\[0.3cm]
  Intelligent des Workflows d'Entreprise}\\[0.4cm]

{\normalsize\itshape\color{titleBlue!80} Cas d'application : agence de location de voitures --- Rabat}

\vspace{0.8cm}

\begin{minipage}{0.72\textwidth}\centering
\begin{tabular}{rl}
\textbf{Intitule du stage :} & \textit{AI Engineer --- automatisation des workflows metier} \\[0.4cm]
\textbf{Encadrant academique :} & \rule{5cm}{0.4pt} \\[0.3cm]
\textbf{Tuteur entreprise :}    & \rule{5cm}{0.4pt} \\[0.3cm]
\textbf{Entreprise d'accueil :} & \rule{5cm}{0.4pt} \\[0.6cm]
\multicolumn{2}{c}{\textbf{Realise par :}} \\[0.3cm]
...
\vspace{0.4cm}
\begin{center}
{\small\color{gray!100}
\textit{Soutenue publiquement le :} \rule{3cm}{0.4pt}}
\end{center}
...
Le prototype livre comporte un \textbf{espace d'administration} en dix ecrans, un \textbf{jeu de donnees realiste} (24 vehicules, 80 reservations, 45 messages sur six semaines) rejoue a travers le vrai graphe, et \textbf{sept simulations metier} automatisees. Evalue sur 45 messages etiquetes, l'agent Intake atteint \textbf{89--91~\% d'exactitude par champ}, et surtout \textbf{100~\% de ses extractions erronees sont interceptees} par la validation humaine. Les indicateurs metier sont encadres pour une mesure avant/apres en pilote, sans chiffre revendique a l'avance.
...
The delivered prototype includes a ten-screen \textbf{admin dashboard}, a \textbf{realistic generated dataset} (24 vehicles, 80 bookings, 45 messages over six weeks) replayed through the real graph, and \textbf{seven automated business simulations}. On 45 labelled messages the intake agent reaches \textbf{89--91~\% per-field accuracy}, and \textbf{100~\% of its errors are caught} by the human gate. Business KPIs are framed for before/after measurement during a pilot, with no figure claimed in advance.
...
Les chiffres cites dans ce rapport sont reproductibles par les commandes \texttt{pytest -q} et \texttt{python -m eval.eval\_intake} dans le depot du projet. Les diagrammes referencés sont disponibles dans \texttt{docs/diagrams/} au format HTML+SVG exportable en PNG/PDF. Les captures d'ecran de l'espace admin proviennent de la V1 livree en septembre 2026.
\end{infobox}
...
\textbf{LangGraph} modelise un workflow comme un graphe dont les noeuds sont des fonctions Python et les aretes sont des decisions conditionnelles. Il offre nativement le \textbf{checkpointing} (persistance par \texttt{SqliteSaver}, \texttt{PostgresSaver}, etc.), l'\textbf{interruption} pour validation humaine (\texttt{interrupt(payload)}) et la reprise par commande (\texttt{Command(resume=decision)} ou \texttt{Command(goto=noeud)}). En septembre 2026, c'est le cadre d'orchestration d'agents le plus adopte de l'ecosysteme Python avec environ 42~000 etoiles GitHub et 44~000 projets dependants. Sa documentation, sa communaute et son integration a l'ecosysteme LangChain (\texttt{init\_chat\_model}, \texttt{with\_structured\_output}) en font un choix de \emph{maturite technique}, non une pari.
...
\caption{Ecran << File a valider >>. Deux onglets (Equipe / Manager), badges de priorite, raisons lisibles, brouillons prets a etre relus. C'est ici que la decision humaine s'exerce.}
...
\caption{Ecran << Suivi en direct >> avec chemin allume et tableau de raisonnement. Le systeme n'est pas une boite noire : chaque decision est traceable a une regle mesuree.}
...
\caption{Ecran << Simulations >>. Sept cas metier avec icone du canal (WhatsApp, Facebook, Instagram, site, telephone, comptoir). Le bouton << Tout jouer >> enchaine les sept cas.}
...
\begin{mechanismbox}[Le filet de securite fonctionne a 100 \%]
Sur les 45 extractions, \textbf{8 contiennent au moins une erreur}. Les \textbf{8/8 sont routees vers un humain} par les seuils (\texttt{confidence\_threshold} = 0{,}75, \texttt{critical\_field\_threshold} = 0{,}7, champs manquants). \emph{Aucune extraction erronee ne sort du systeme en autonome.}
\end{mechanismbox}

\subsection{La calibration de la confiance}
\begin{itemize}[leftmargin=1.5cm]
\item Confiance $\geq$ 0{,}8 $\longrightarrow$ \textbf{94~\%} d'extractions entierement correctes.
\item Confiance $<$ 0{,}8 $\longrightarrow$ \textbf{0~\%} d'extractions entierement correctes.
\end{itemize}
Le seuil est \emph{bien place} : ce qui passe en automatique est fiable, ce qui doute est relu.
...
L'evaluation demontre que le prototype respecte l'integralite des exigences fonctionnelles (16 tests verts), que l'agent Intake atteint 89--91~\% d'exactitude avec un filet de securite de 100~\%, que sa calibration est correcte, et que la robustesse a ete verifiee sur six cas de defaillance. Les indicateurs metier sont cadres pour une mesure honnete en pilote. Le chapitre suivant discute les limites, l'ethique, la generalisation du systeme et les perspectives ouvertes par ce travail.
...
\item LangChain Inc., \emph{LangGraph --- StateGraph, persistence, human-in-the-loop, interrupts}, 2026. \url{https://github.com/langchain-ai/langgraph}
\item Microsoft, \emph{Agent Framework --- workflows, checkpointing, observability}, 2026. \url{https://github.com/microsoft/agent-framework}
\item A. Perritano, \emph{LangGraph Customer Support Agent}, 2025.
\item E. Surovtsev, \emph{LangGraph HITL FastAPI demo}, 2025.
\item E. Cingoz, \emph{Awesome n8n Templates}, 2026. \url{https://github.com/enescingoz/awesome-n8n-templates}
\item TypeSafe, \emph{Jev AI --- typed decisions with calibrated confidence}, 2026. \url{https://jev-ai.pro}
\item Anthropic, \emph{Model Context Protocol Specification}, 2025--2026.
\item Y. Hu, Y. Koren, C. Volinsky, \emph{Collaborative Filtering for Implicit Feedback Datasets}, IEEE ICDM 2008.
\item FastAPI, SQLAlchemy 2.0, React 19, Vite, TypeScript --- documentations officielles, 2026.
\item Simon Brown, \emph{The C4 Model for Software Architecture}, 2018--2026. \url{https://c4model.com}
\item Cocoon AI, \emph{Architecture Diagram Generator}, 2026.
...
\item Anthropic, \emph{Claude Docs and Model Cards (Claude 5 family, Haiku 4.5)}, 2026.
\end{enumerate}
